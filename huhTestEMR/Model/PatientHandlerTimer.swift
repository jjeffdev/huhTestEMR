...
class PatientHandlerTimer: ObservableObject {
     ...
     */
    init(lengthInMinutes: Int = 0, attendees: [PatientInfo.PatientHandler] = []) {
     ...
     */
    func reset(lengthInMinutes: Int, attendees: [PatientInfo.PatientHandler]) {
        self.lengthInMinutes = lengthInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension PatientInfo {
    ...
    var timer: PatientHandlerTimer {
        PatientHandlerTimer(lengthInMinutes: faceToFaceMinutes, attendees: patientHandlers)
    }
}

extension Array where Element == PatientInfo.PatientHandler {
    var speakers: [PatientHandlerTimer.Speaker] {
        if isEmpty {
            return [PatientHandlerTimer.Speaker(name: "Speaker 1", isCompleted: false)]
        } else {
            return map { PatientHandlerTimer.Speaker(name: $0.name, isCompleted: false) }
        }
    }
}
